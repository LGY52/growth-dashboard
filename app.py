...
dfs = []
for f in uploaded:
    df_tmp = pd.read_excel(io.BytesIO(f.read()), engine='openpyxl')
    dfs.append(df_tmp)
...
extract_year = data['직무연차'].str.extract(r'(\d+)')[0].astype(int)
data['경력그룹'] = pd.cut(extract_year, bins=bins, labels=labels, right=False)
