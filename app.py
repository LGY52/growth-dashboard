...
stopwords = {
    ...
    '되어','될','했습니다','입니다','위해','희망','분야의도'
}
ending_pattern = re.compile(
    r'(다|합니다|입니다|이다|한다|었|했|어요|에|하고|함으로서|하여|으로는|니까|여서|이므로|바탕으로)$'
)
# 조사·접미사까지 모두 동일어로 묶기 위한 정규표현식
norm_pattern = re.compile(r'(으로는|으로|로는|로|와|과|이|가|은|는)$')
